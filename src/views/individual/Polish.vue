  ...
  <template v-if="!formShow">
    <div class="container">
      ...
      <div class="bottom">
        <button
          class="common-button"
          :class="{'common-button-disabled': !isFileUploaded}"
          @click="polishResume"
        >
          开始润色
        </button>
...
const isFileUploaded = ref(false); // 新增状态来跟踪文件上传状态
...
function handleDrop(e) {
  ...
  if (file && file.type === 'application/pdf') {
    ...
    isFileUploaded.value = true; // 上传文件时设置状态
    uploadFile(); // 调用上传文件函数
  } else {
    ElMessage({
      showClose: true,
...
async function uploadFile() {
  ...
  try {
    ...
    isFileUploaded.value = true; // 上传成功后设置状态
...
async function polishResume() {
  ...
  try {
    // 调用润色接口
    const response = await axios.post('/resume/polish', {
      // 传递必要的参数
    }, {
      headers: {
        'Authorization': 'JWT ' + localStorage.getItem('token'), // 假设 JWT 存储在 localStorage 中
    ...
    console.log('润色请求成功:', response.data);
  } catch (error) {
    ElMessage({
      showClose: true,
      message: '润色请求失败！',
